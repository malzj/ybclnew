...
<div class="yzf_01">
    ...
    <div class="yzf_06_bot">
        <p style="background:#48bbb4; color:#fff;"><span>生日</span><span>车辆信息</span><span>公司</span><span>服务商</span><span>姓名</span></p>
        <g:each in="${fwsClientList}" status="i" var="userInstance">
            <g:link action="ybClientShow" target="_self" id="${userInstance?.birthday}"><span>${userInstance?.carMessage}</span><span>${userInstance?.company}</span><span>${userInstance?.fwsShop}</span><span>${userInstance?.name}</span></g:link>
