...
<div class="yzf_01">
    ...
    <div class="yzf_07_bot">
        ...
        <g:each in="${fwsGongNengInstanceList}" status="i" var="gnInstance">
            <g:link action="fwsGongNengShow" target="_self" id="${gnInstance?.id}"><span>${gnInstance.id}</span><span>${gnInstance.functionName}</span><span>${gnInstance.functionLink}</span><span>${gnInstance.time}</span>
